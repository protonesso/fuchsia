...
zx_status_t MsiDispatcher::Create(fbl::RefPtr<MsiAllocation> alloc, uint32_t msi_id,
  ...
  auto* cap = reinterpret_cast<MsiCapability*>(mapping->base() + reg_offset);
  ...
  switch (cap->id) {
    case kMsiCapabilityId: {
      // MSI capabilities fit within a given device's configuration space which
      // happens to match the smallest possible VMO size.
      if (vmo->size() != ZX_PAGE_SIZE || reg_offset > vmo->size() - sizeof(MsiCapability)) {
        return ZX_ERR_INVALID_ARGS;
      }
      disp = fbl::AdoptRef<MsiDispatcher>(
          new (&ac) MsiDispatcherImpl(ktl::move(alloc), base_irq_id, msi_id, ktl::move(mapping),
                                      reg_offset, has_pvm, has_64bit, register_int_fn));
    } break;
    default:
